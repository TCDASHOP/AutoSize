...
(() => {
  ...
  const i18n = {
    jp: {
      ...
      labelFootLen: "足長（cm）",
      hintFootLen: "左右を測って長い方を入力してください（下で左右も記録できます）。",
      labelFootSide: "長い方",
      hintFootSide: "任意：左／右どちらが長いか（ログ用）。",
      labelFitShoe: "好み（捨て寸）",
      ...
      btnExportLogs: "ログを書き出す",
      btnClearLogs: "ログを消す",
      logInfo: "ログ：{n}件",
      confirmClearLogs: "ログをすべて削除しますか？",
      msgExported: "ログを書き出しました。",
    ...
    en: {
      ...
      hintFootLen: "Measure both feet and use the longer one (you can log the side below).",
      labelFootSide: "Longer foot",
      hintFootSide: "Optional: left/right (for logging).",
...
function updateLogInfo(){
  if (!els.logInfo) return;
  const logs = readLogs();
  const tt = t();
  const fmt = tt.logInfo || "Logs: {n}";
  els.logInfo.textContent = fmt.replace("{n}", String(logs.length));
}
...
function logsToCsv(logs){
  const cols = [
    "ts","lang","unit","productKey","productType","productName","inputMethod",
    "nudeChest","estimatedChest","height","weight","sex","bmi",
    "fitTop","topPriority",
    "footLen","footSide","footWidth","fitShoe",
    "recommended","runnerUp","detail","error"
  ];
  const esc = (v) => {
    if (v == null) return "";
    const s = String(v);
  ...
  logs.forEach(L => {
    ...
    const o = {
      ...
      nudeChest: i.nudeChest ?? "",
      estimatedChest: i.estimatedChest ?? "",
      height: est.height ?? "",
      weight: est.weight ?? "",
      sex: est.sex ?? "",
      bmi: est.bmi ?? "",
      ...
      footLen: i.footLen ?? "",
      footSide: i.footSide ?? "",
      footWidth: i.footWidth ?? "",
      fitShoe: i.fitShoe ?? "",
      recommended: r.recommended ?? r.size ?? "",
      runnerUp: r.runnerUp ?? "",
      detail: r.detail ?? "",
      error: L.error ?? ""
    };
    rows.push(cols.map(c => esc(o[c])).join(","));
  });
  return rows.join("\n");
}
...
function exportLogs(){
  ...
  const base = `tcda_sizeguide_logs_${stamp}`;
  downloadText(`${base}.json`, JSON.stringify(logs, null, 2));
  downloadText(`${base}.csv`, logsToCsv(logs));
}
...
function runnerUpLabelTops(primary, alt){
  ...
  const unitIsCm = (state.unit === "cm");
  if (primary?.idx != null && alt?.idx != null && alt.idx === primary.idx){
    return isJP
      ? `次点：${alt.size}（他サイズが無い場合）`
      : `Runner-up: ${alt.size} (only one size available)`;
  }
  const lenThr = unitIsCm ? 1.0 : 0.5; // minimal meaningful length difference
  const hasLen = Number.isFinite(primary?.len) && Number.isFinite(alt?.len);
  const hasChest = Number.isFinite(primary?.half) && Number.isFinite(alt?.half);

  const lenDiff = hasLen ? (alt.len - primary.len) : NaN;
  const chestDiff = hasChest ? (alt.half - primary.half) : NaN;

  // Prefer describing the *reason* for the runner-up.
  // If we can see a length difference, it’s usually the "length vs chest" toggle case.
  if (hasLen && Math.abs(lenDiff) >= lenThr){
    if (state.topPriority === "chest"){
      // runner-up represents length preference
      return isJP
        ? `次点：${alt.size}（着丈優先なら）`
        : `Runner-up: ${alt.size} (if you prioritize length)`;
    } else {
      // runner-up represents chest preference
      return isJP
        ? `次点：${alt.size}（胸囲優先なら）`
        : `Runner-up: ${alt.size} (if you prioritize chest)`;
    }
  }

  // Otherwise, describe it as a fit tradeoff (looser vs snugger).
  if (hasChest && Math.abs(chestDiff) > 0){
    const looser = chestDiff > 0;
    return isJP
      ? `次点：${alt.size}（${looser ? "ゆったりなら" : "ぴったりなら"}）`
      : `Runner-up: ${alt.size} (${looser ? "looser" : "snugger"} fit)`;
  }

  return isJP ? `次点：${alt.size}` : `Runner-up: ${alt.size}`;
}
...
function recommendTopsMulti(rows){
  ...
  // Ensure alt exists & differs (always show 2 options when possible)
  if (!alt){
  ...
  if (alt && alt.idx === primary.idx){
    alt = neighborInSorted(byChest.sorted, primary.idx, +1) || neighborInSorted(byChest.sorted, primary.idx, -1) || null;
  }
  if (!alt) alt = primary;
...
function recommendShoesMulti(rows){
  ...
  // runner-up direction (always show 2 options when possible)
  let alt = null;
  let altLabel = "";
  ...
  if (width === "slim"){
    ...
    if (alt){
      altLabel = (state.lang === "jp")
        ? (alt === nextDown
            ? `次点：${display(alt)}（細めなら）`
            : `次点：${display(alt)}（細めでも小さいサイズが無い場合）`)
        : (alt === nextDown
            ? `Runner-up: ${display(alt)} (if your feet are slim)`
            : `Runner-up: ${display(alt)} (no smaller size available)`);
    }
  } else if (width === "wide"){
    alt = nextUp || nextDown;
    if (alt){
      altLabel = (state.lang === "jp")
        ? (alt === nextUp
            ? `次点：${display(alt)}（幅広/厚手ソックスなら）`
            : `次点：${display(alt)}（幅広でも大きいサイズが無い場合）`)
        : (alt === nextUp
            ? `Runner-up: ${display(alt)} (wide feet / thick socks)`
            : `Runner-up: ${display(alt)} (no larger size available)`);
    }
  } else {
    // standard
    alt = nextUp || nextDown;
    if (alt){
      altLabel = (state.lang === "jp")
        ? (alt === nextUp
            ...
            : `次点：${display(alt)}（よりフィットなら）`)
        : (alt === nextUp
            ? `Runner-up: ${display(alt)} (thick socks / looser fit)`
            : `Runner-up: ${display(alt)} (for a snugger fit)`);
    }
  }

  // If we still have no runner-up (rare), show the same size with a note.
  if (!alt){
    alt = chosen;
    altLabel = (state.lang === "jp")
      ? `次点：${display(alt)}（他サイズが無い場合）`
      : `Runner-up: ${display(alt)} (only one size available)`;
  }
...
function recommendTops(rows){
  ...
  function applyI18n(){
    ...
    if (els.footSide){
      ...
      opts.forEach(opt => {
        if (opt.value === "unknown") opt.textContent = (state.lang === "jp") ? "未選択" : "Not set";
        if (opt.value === "left") opt.textContent = (state.lang === "jp") ? "左が長い" : "Left";
        if (opt.value === "right") opt.textContent = (state.lang === "jp") ? "右が長い" : "Right";
        if (opt.value === "same") opt.textContent = (state.lang === "jp") ? "同じ" : "Same";
      });
    }
    ...
    if (els.btnExportLogs) els.btnExportLogs.textContent = tt.btnExportLogs;
    if (els.btnClearLogs) els.btnClearLogs.textContent = tt.btnClearLogs;
    updateLogInfo();
  ...
  if (p.type === "shoes"){
    ...
    if (rec.error){
      ...
      pushLog({
        ...
        productKey: state.productKey,
        productType: p.type,
      productName: (state.lang === "jp" ? p.labelJP : p.labelEN).join(" "),
        inputs: {
          footLen: els.footLen?.value ?? "",
          footSide: els.footSide?.value ?? "unknown",
          footWidth: els.footWidth?.value ?? "standard",
          fitShoe: els.fitShoe?.value ?? "",
        }
      });
      return;
    }
    ...
    pushLog({
      ...
      inputs: {
        ...
        estimatedChest: rec.usedEst ? rec.estCm : null,
        fitTop: els.fitTop?.value ?? "",
        topPriority: state.topPriority,
        estimator: {
          height: rec.heightCm,
          weight: toNumber(els.estWeight?.value),
          sex: els.estSex?.value ?? "",
...
// ---------- events ----------
  ...
  function bindEvents(){
    ...
    // log tools
    els.btnExportLogs?.addEventListener("click", () => {
      exportLogs();
      if (els.logInfo){
        els.logInfo.textContent = t().msgExported;
        setTimeout(() => updateLogInfo(), 1400);
      }
    });
    els.btnClearLogs?.addEventListener("click", () => {
      const ok = confirm(t().confirmClearLogs);
      if (!ok) return;
      clearLogs();
    });
